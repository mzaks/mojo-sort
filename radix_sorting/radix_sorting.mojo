...
fn _get_index[D: DType, place: Int](vector: List[Scalar[D]], v_index: Int) -> Int:
    @parameter
    if D == DType.int8:
        return Int((bitcast[DType.uint8, 1](vector.unsafe_get(v_index)) ^ last_bit_8) >> place) & 255
    elif D == DType.int16:
        return Int((bitcast[DType.uint16, 1](vector.unsafe_get(v_index)) ^ last_bit_16) >> place) & 255
    elif D == DType.float16 or D == DType.bfloat16:
        var f = bitcast[DType.uint16, 1](vector.unsafe_get(v_index))
        var mask = bitcast[DType.uint16, 1](-bitcast[DType.int16, 1](f >> 15) | last_bit_16)
        return Int((f ^ mask) >> place) & 255
    elif D == DType.int32:
        return Int((bitcast[DType.uint32, 1](vector.unsafe_get(v_index)) ^ last_bit_32) >> place) & 255
    elif D == DType.float32:
        var f = bitcast[DType.uint32, 1](vector.unsafe_get(v_index))
        var mask = bitcast[DType.uint32, 1](-bitcast[DType.int32, 1](f >> 31) | last_bit_32)
        return Int((f ^ mask) >> place) & 255
    elif D == DType.int64:
        return Int((bitcast[DType.uint64, 1](vector.unsafe_get(v_index)) ^ last_bit_64) >> place) & 255
    elif D == DType.float64:
        var f = bitcast[DType.uint64, 1](vector.unsafe_get(v_index))
...
fn _counting_sort[D: DType, CD:DType, place: Int](mut vector: List[Scalar[D]]):
    ...
    while i >= 0:
        var index = _get_index[D, place](vector, i)
        output.unsafe_get(Int(counts.offset(index).load() - 1)) = vector.unsafe_get(i)
        counts.offset(index).store(counts.offset(index).load() - 1)
        i -= 1
    vector = output^
